...
if (!accessToken || !deviceId || !projectId || !outputPath) {
    console.error('Usage: node captureSnapshot.js <accessToken> <deviceId> <projectId> <outputPath>');
    process.exit(1);
}
...
async function callNestApi(offerSdp) {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            ...
            port: 443,
            path: `/v1/enterprises/${projectId}/devices/${deviceId}:executeCommand`,
            method: 'POST',
            headers: {
...
async function main() {
    ...
    try {
        ...
        const answerSdp = await callNestApi(offerSdp);
